...
km_out <- kmeans(sim_data, centers = 4, nstart = 20)
km_out
km_out$cluster
...
#dist(sim_data, method = "euclidean")
...
hc_centroid <- hclust(dist(sim_data), method = "centroid")
plot(hc_centroid)
...
cutree(hc_average, 4)
cutree(hc_centroid,4)

ggplot(sim_data, aes(x, y)) +
  geom_point(shape = cutree(hc_complete, 4), size = 2)
ggplot(sim_data, aes(x, y)) +
  geom_point(shape = cutree(hc_centroid, 4), size = 2)
ggplot(sim_data, aes(x, y)) +
  geom_point(shape = cutree(hc_average, 4), size = 2)
